...
#define FLIP_BIT(_ar, _b) do { \
  ...
  FILE *fp;
  fp = fopen("out_buf_3.js", "w+");
  fputs(out_buf, fp);
  fclose(fp);

  int cur_samples_num = mutate_senmantic_all(out_buf, len, 4000, 10000);
  char *res = NULL;
  size_t ret_len = 0;
  char *out_buf1 = malloc(10000);

  for(i=0;i<cur_samples_num;i++){
    fuzz(i, &res, &ret_len);
    memcpy(out_buf1, res, ret_len);
    if (common_fuzz_stuff(argv, out_buf1, ret_len)) goto abandon_entry;
  }
...
int main(int argc, char** argv) {
  ...
  char exp_path[]="/home/keven/Projects/Fuzzing/Superion/tree_mutation/js_parser_v2/init_exps_all";
  init_exp_rules(exp_path);
  char st_path[]="/home/keven/Projects/Fuzzing/Superion/tree_mutation/js_parser_v2/init_st_all";
  init_st_rules(st_path);

...
int main()
{
	...
	int target_state = 43;
	for(int i=0;i<text_pool[target_state].size() && i<100;i++){
		cout<<i<<": "<<text_pool[target_state][i]<<endl;
	}

	// target_state = 43;
	// for(int i=0;i<text_pool[target_state].size();i++){
	// 	cout<<i<<": "<<text_pool[target_state][i]<<endl;
	// }
	exit(0);
	...
	char *s_buf=NULL;
	size_t s_len = 0;
	mutate_one_local(target, len, 0, &s_buf, &s_len);
	cout << s_buf << endl;
	cout << s_len << endl;
	// mutate_one_local(target, len, 0, &s_buf, &s_len);
	// cout << s_buf << endl;

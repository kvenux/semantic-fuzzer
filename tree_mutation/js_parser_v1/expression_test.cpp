...
int mutate_one(char *target, size_t len, int mutate_loc){
	...
	// int rep_text_index = text_pool[cur_rule_index].size() - 1;
	// cout<<"rep_text_index: "<<rep_text_index<<endl;
...
int mutate_one_local(char *target, size_t len, int mutate_loc, char **result, size_t *retlen){
	try{
		...
		if(text_pool[cur_rule_index].size() == 0){
			cout<<"cur_rule_index"<<cur_rule_index<<endl;
			cout<<"no candidates"<<endl;
			*retlen = targetString.length();
			*result = strdup(targetString.c_str());
			return 0;
		}
		int rep_text_index = rand() % text_pool[cur_rule_index].size();
		// if(cur_rule_index == 43){
		// 	if(rand()%2 == 0)
		// 		rep_text_index = 1;
		// 	else
		// 		rep_text_index = 4;
		...
		rewriter.replace(intervals[mutate_loc].a, intervals[mutate_loc].b, text_pool[cur_rule_index][rep_text_index]);
		cout<<rewriter.getText()<<endl;
		// cout<<"rewriter.getText().length();"<<rewriter.getText().length()<<endl;
...
int main()
{
	...
	int len = 0;
	in.open("/home/keven/Projects/Fuzzing/Superion/tree_mutation/js_parser/exp_1.js");
	while (!in.eof())
	{
	...
	int lenS = 0;
	in.open("/home/keven/Projects/Fuzzing/Superion/tree_mutation/js_parser/exp_2.js");
	while (!in.eof())
	{
	...
	for(int i=0;i<10;i++){
		...
		mutate_one_local(target, len, rand()%5, &res, &ret_len);
		// cout<<"ret_len: "<<ret_len<<endl;
		memcpy(target, res, ret_len);
		len = ret_len;
		// cout<<target<<endl;
	}
	// mutate_one(target, len, 3);

...
extern "C" void init_st_rules(char *path);
extern "C" void init_exp_rules(char *path);
extern "C" int mutate_senmantic_all(char *target, size_t len, int st_mut_num, int exp_mut_num);


#define MAXSAMPLES 50000
#define MAXTEXT 200
#define MAX_EXP_MUTATION_LEN 1000
#define MAX_ST_MUTATION_LEN 400
...
int mutate_senmantic_all(char *target, size_t len, int st_mut_num, int exp_mut_num){
	...
	try{
		...
		int exp_size = visitor->exp_texts.size();
		int uniform_size = st_size + exp_size;
		// int exp_mut_num = (MAXSAMPLES - st_list.size() * st_size) / exp_size;
		int st_mut_num_each = st_mut_num / (st_size+1) + 1;
		int exp_mut_num_each = exp_mut_num / (exp_size+1) + 1;
		int max_samples = st_mut_num + exp_mut_num;
		int sample_num_st_mut = 0;
		int sample_num_exp_mut = 0;

		vector<string> candidates;
		string res;
		// for each statement
		for(int st_index=0;st_index<st_size;st_index++){
			candidates.clear();
			int cur_end_pos = visitor->st_ends[st_index];
			for(int i=0;i<visitor->decl_id_texts.size();i++){
				if(visitor->decl_ends[i] < cur_end_pos){
					candidates.push_back(visitor->decl_id_texts[i]);
				}
			...
			if(candidates.size() == 0 || st_list.size() == 0){
				continue;
			}
			for(int rep_index=0;rep_index<st_list.size() && rep_index < MAX_ST_MUTATION_LEN;rep_index++){
				if(num_of_smaples>=max_samples || sample_num_st_mut>=st_mut_num)break;
				res = st_list[rep_index].mutator(candidates);
				if(visitor->st_intervals[st_index].a > visitor->st_intervals[st_index].b)
					continue;
				int RandIndex = rand() % (st_list.size());
				if(RandIndex > st_mut_num_each)
					continue;
				rewriter.replace(visitor->st_intervals[st_index].a, visitor->st_intervals[st_index].b, res);
				// cout<<res<<endl;
				// cout<<st_index<<" "<<rep_index<<" "<<res.size()<<endl;
				ret[num_of_smaples++]=rewriter.getText();
				sample_num_st_mut++;
				if(num_of_smaples>=max_samples)break;
			}
		}

		cout<<"st num_of_smaples "<<num_of_smaples<<endl;

		// for each expression
		// cout<<"exp_size"<<exp_size<<endl;
		// cout<<"exp_list.size()"<<exp_list.size()<<endl;

		for(int exp_index=0;exp_index<exp_size;exp_index++){
			candidates.clear();
			int cur_end_pos = visitor->exp_ends[exp_index];
			for(int i=0;i<visitor->decl_id_texts.size();i++){
				if(visitor->decl_ends[i] < cur_end_pos){
					candidates.push_back(visitor->decl_id_texts[i]);
				}
			...
			if(candidates.size() == 0 || exp_list.size() == 0){
				continue;
			}

			for(int rep_index=0;rep_index<exp_list.size() && rep_index < MAX_EXP_MUTATION_LEN;rep_index++){
				if(num_of_smaples>=max_samples || sample_num_exp_mut >= exp_mut_num)break;
				res = exp_list[rep_index].mutator(candidates);
				if(visitor->exp_intervals[exp_index].a > visitor->exp_intervals[exp_index].b)
					continue;
				int RandIndex = rand() % (exp_list.size());
				// cout<<"RandIndex"<<RandIndex<<endl;
				// cout<<"exp_mut_num_each"<<exp_mut_num_each<<endl;
				if(RandIndex > 50*exp_mut_num_each)
					continue;
				rewriter.replace(visitor->exp_intervals[exp_index].a, visitor->exp_intervals[exp_index].b, res);
				// cout<<rewriter.getText()<<endl;
				// cout<<exp_index<<" "<<rep_index<<" "<<res.size()<<endl;
				ret[num_of_smaples++]=rewriter.getText();
				sample_num_exp_mut++;
			}
		}
...
int parse_nlevel(char *target, size_t len, const char* name, char* path){
	int num_of_smaples = 0;
	// auto t1 = std::chrono::high_resolution_clock::now();
	// auto t2 = std::chrono::high_resolution_clock::now();
	// auto duration = std::chrono::duration_cast<std::chrono::microseconds>( t2 - t1 ).count();
    ...
    vector<string> int_texts;
    int interval_cnt = st_size + visitor->fun_starts.size();
    //interval_cnt += visitor->decl_starts.size();
    intervals.resize(interval_cnt);
    int_texts.resize(interval_cnt);
    int cur_size = 0;
    for(auto i=0;i<st_size;i++){
      //cout<<visitor->st_starts[i]<<" "<<visitor->st_ends[i]<<endl;
      //cout<<visitor->st_texts[i]<<endl;
      intervals[cur_size + i].resize(3);
      intervals[cur_size + i][0] = visitor->st_starts[i];
      intervals[cur_size + i][1] = visitor->st_ends[i];
      intervals[cur_size + i][2] = cur_size + i;
      int_texts[cur_size + i] = visitor->st_texts[i];
		}
    cur_size += st_size;
    for(auto i=0;i<visitor->fun_starts.size();i++){
      intervals[cur_size + i].resize(3);
      intervals[cur_size + i][0] = visitor->fun_starts[i];
      intervals[cur_size + i][1] = visitor->fun_ends[i];
      intervals[cur_size + i][2] = cur_size + i;
      int_texts[cur_size + i] = visitor->fun_texts[i];
    }
    //cur_size += visitor->fun_starts.size();
    //for(auto i=0;i<visitor->decl_starts.size();i++){
    //intervals[cur_size + i].resize(3);
    //intervals[cur_size + i][0] = visitor->decl_starts[i];
    //intervals[cur_size + i][1] = visitor->decl_ends[i];
    //intervals[cur_size + i][2] = cur_size + i;
    //int_texts[cur_size + i] = visitor->decl_texts[i];
    //}
    ...
    char dst[100];
    ofstream outf;
    for(auto i=1;i<res.size() && i<=10;i++){
      //cout<<res[i][0]<<" "<<res[i][1]<<endl;
      //cout<<int_texts[res[i][2]]<<endl;
      //cout<<targetString.substr(0, res[i-1][0])<<endl;
      snprintf(dst, sizeof(dst), "%s/%s-%d.js", path, name, i);
      //cout<<dst<<" "<<targetString.substr(0, res[i-1][0]).size()<<endl;
      outf.open(dst);
      outf<<targetString.substr(0, res[i-1][0]);
      outf.close();
    }
...
void init_exp_rules(char *path){
	...
	vector<string> exps;
	splitString(exps, buffer.str(), "&$%");
	cout<<"exps.size() "<<exps.size()<<endl;

	int rules_num = 0;
	for(int i=0;i<exps.size();i++){
		// cout<<exps[i]<<endl;
		vector<string> cur_exp_vec;
		splitString(cur_exp_vec, exps[i], "@#@");

		string exp_text = cur_exp_vec[0];
		if(exp_text.size() > 10)  continue;
		// cout<<exp_text<<endl;

		vector<string> id_info_vec;
		splitString(id_info_vec, cur_exp_vec[1], ",");
		// cout<<i<<": "<<cur_exp_vec.size()<<endl;
		// cout<<"exp_text: "<<exp_text<<endl;
		// cout<<"cur_exp_vec[1]: "<<cur_exp_vec[1]<<endl;
		// cout<<"id_info_vec.size(): "<<id_info_vec.size()<<endl;

		exp_list.push_back(ExpToReplace(exp_text));
		for(int j=0;j<id_info_vec.size()/3;j++){
			string a = "25";
			// cout<<id_info_vec[j]<<endl;
			int cur_id_start = atoi(id_info_vec[j*3].c_str());
			// cout<<cur_id_start<<endl;
			int cur_id_end = atoi(id_info_vec[j*3+1].c_str());
			// cout<<id_info_vec[j+2]<<endl;
			// int cur_id_end = stoi(id_info_vec[j+1]);
			string cur_id_text = id_info_vec[j*3+2];
			exp_list.back().addID(cur_id_start, cur_id_end, cur_id_text);
		}

	}

	cout<<"init exp_list.size "<<exp_list.size()<<endl;
}

void init_st_rules(char *path){
	ifstream in_fs(path);
	stringstream buffer;
	buffer << in_fs.rdbuf();
	vector<string> statements;
	splitString(statements, buffer.str(), "&$%");

	cout<<"statements.size() "<<statements.size()<<endl;
	int rules_num = 0;
	for(int i=0;i<statements.size();i++){
		vector<string> cur_st_vec;
		splitString(cur_st_vec, statements[i], "@#@");

		string st_text = cur_st_vec[0];
		// cout<<st_text<<endl;

		vector<string> id_info_vec;
		splitString(id_info_vec, cur_st_vec[1], ",");
		st_list.push_back(StatementToReplace(st_text));
		for(int j=0;j<id_info_vec.size()/3;j++){
			int cur_id_start = atoi(id_info_vec[j*3].c_str());
			int cur_id_end = atoi(id_info_vec[j*3+1].c_str());
			string cur_id_text = id_info_vec[j*3+2];
	...
	cout<<"init st_list.size "<<st_list.size()<<endl;
}
...
int main(int argc, char** argv)
{
  ...
  //init_exp_rules("init_exps_all");
  //init_st_rules("init_st_all");
  string src = argv[1];
  //cout<<src.substr(0, src.find("."))<<endl;

  parse_nlevel(target, len, argv[2], argv[3]);
	exit(0);
}

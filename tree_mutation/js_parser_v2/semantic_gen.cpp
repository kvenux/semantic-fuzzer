...
int main()
{
	...
	string path = "/home/keven/Projects/Fuzzing/jerryscript/init_seeds_var_100/";
	DIR *dir = opendir("/home/keven/Projects/Fuzzing/jerryscript/init_seeds_var_100");
	if (dir == NULL) {
		return 0;
	}
	...
	for(int i=0;i<st_list.size();i++){
		cout<<i<<" "<<st_list.size()<<endl;
		// cout<<exp_list[i].text<<endl;
		if(i>= st_list.size()) break;
		string res_i = st_list[i].mutator(candds);
		for(int j=i+1;j<st_list.size();j++){
			if(j>= st_list.size()) break;
			string res_j = st_list[j].mutator(candds);
			if(res_i.compare(res_j) == 0){
				// cout<<res_i<<" "<<res_i<<endl;
				st_list.erase(st_list.begin() + j);
			}
		}
	}
	...
	string out_buf = "";
	for(int i=0;i<st_list.size();i++){
		// cout<<st_list[i].text<<endl;
		// string cur_exp_buf = "";
		string cur_buf = "";
		cur_buf += st_list[i].text;
		cur_buf += "@#@";
		for(int j=0;j<st_list[i].id_texts.size();j++){
			cur_buf += to_string(st_list[i].id_starts[j]) + ",";
			cur_buf += to_string(st_list[i].id_ends[j]) + ",";
			cur_buf += st_list[i].id_texts[j] + ",";
		}
		cur_buf += "@#@";
		cur_buf += "&$%";
		// cout<<cur_buf<<endl;
	...
	// int cur_index = 202;
	// cout<<st_list[cur_index].text<<endl;
	// cout<<"id_texts.size() "<<st_list[cur_index].id_texts.size()<<endl;
	// for(int j=0;j<st_list[cur_index].id_texts.size();j++){
	// 	cout<<st_list[cur_index].id_starts[j]<<" ";
	// 	cout<<st_list[cur_index].id_ends[j]<<" ";
	// 	cout<<st_list[cur_index].id_texts[j]<<" ";
	// 	cout<<endl;
	// }

	ofstream fout;
	// fout.open("init_exps_all");
	fout.open("init_st_all");
	fout<<out_buf;
	fout.close();
	cout<<st_list.size()<<endl;

...
int mutate_senmantic_all(char *target, size_t len, string path){
	int num_of_smaples = 0;
	// auto t1 = std::chrono::high_resolution_clock::now();
	// auto t2 = std::chrono::high_resolution_clock::now();
	// auto duration = std::chrono::duration_cast<std::chrono::microseconds>( t2 - t1 ).count();
...
int main()
{
  ...
  //int cnt = mutate_senmantic_all(target, len, 2, 10);
  ...
  //}
	...
	char *s_buf=NULL;
	size_t s_len = 0;
	mutate_exp_and_st(target, len, 13, &s_buf, &s_len);
	cout << s_buf << endl;
	cout << s_len << endl;
	mutate_exp_and_st(s_buf, s_len, 23, &s_buf, &s_len);
	cout << s_buf << endl;
	cout << s_len << endl;
	mutate_exp_and_st(s_buf, s_len, 33, &s_buf, &s_len);
	cout << s_buf << endl;
	cout << s_len << endl;
	exit(0);
}
